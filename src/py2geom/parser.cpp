...
std::string format_svg_pathvector(Geom::PathVector* pv, int prec = -1, bool optimize = false, bool shorthands = true) {
    return Geom::write_svg_path(*pv, prec, optimize, shorthands);
}
BOOST_PYTHON_FUNCTION_OVERLOADS(format_svg_pathvector_overloads, format_svg_pathvector, 1,4)

std::string format_svg_path(Geom::Path* path, int prec = -1, bool optimize = false, bool shorthands = true) {
    Geom::PathVector pv;
    pv.push_back(*path);
    return Geom::write_svg_path(pv, prec, optimize, shorthands);
}
...
void wrap_parser() {
    ...
    def("format_svg_path", format_svg_pathvector, format_svg_pathvector_overloads());
