...
PointVec ch_boundary(Geom::ConvexHull const &ch) {
    return PointVec(ch.begin(), ch.end());
}
...
void wrap_convex_hull() {
    class_<Geom::ConvexHull>("ConvexHull", init<>())
        ...
        .add_property("singular", &Geom::ConvexHull::isSingular)
        .add_property("linear", &Geom::ConvexHull::isLinear)
        .add_property("is_degenerate", &Geom::ConvexHull::isDegenerate)
